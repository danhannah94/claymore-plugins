...
**Consult Dev Memory (when configured).** In autri-equipped environments a **Dev Memory KB** holds a distilled episode of every prior session — its decisions, the *why* behind them, and the steering that shaped them. **Make it one of the first places you look, not a last resort** — at session start and before any non-trivial or easily-relitigated decision, query it (`autri filter "<topic>" --rank relevance_recency` for the freshest *relevant* rationale, or `autri filter --rank recency` for what's most recent — see the project's `autri-api` skill) and build on what a past session already settled instead of re-deriving it. `/hl:start` recalls recent + on-topic memory right after `next.md` and ahead of the local ledgers, so they corroborate it rather than substitute for it; `/hl:stop` records the session back into it. The memory compounds only if you READ it, not just write it. Where no Dev Memory is configured (e.g. a work machine without autri) this principle is simply inert — the methodology is unchanged.
