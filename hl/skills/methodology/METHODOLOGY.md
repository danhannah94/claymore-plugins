...
**Consult Dev Memory (when configured).** In autri-equipped environments a **Dev Memory KB** holds a distilled episode of every prior session — its decisions, the *why* behind them, and the steering that shaped them. **Make it one of the first places you look, not a last resort.** When you're running inside an MCP client with the Autri connector attached, **prefer the Dev Memory MCP tools directly** (`filter_then_rank` / `vector_search` / `get_document`) — you're an MCP client, so just call them. The `pnpm recall "<question>"` verb (run from the autri repo root — hybrid vector ∪ fts, human-readable; `pnpm recall --recent` for "where were we") is the CLI fallback for when no connector is attached. More broadly: **reach for whatever MCP tools are attached, liberally, to fill gaps rather than guess** — Dev Memory is the first instance of the habit, not the only one. **Reach for it on concrete triggers, not just at session start — run `recall` the moment you would otherwise:** (a) **make an assumption** about a past decision, a constraint, or why something is the way it is (recall instead of guessing); (b) **ask the human a question** — odds are a prior session already settled it, so recall first and only ask if it genuinely isn't there; (c) **grep the codebase for the _why_** — grep finds the *what*, so precede it with (or pair it with) a `recall` for the rationale; or (d) hit a **why/how gap** ("why did we pick X?", "what was the reasoning behind that constraint?") or a **"have we hit this before?"** question. Still cross-check `decisions.md` and the code — but `recall` FIRST: its unique value is the rationale and discarded alternatives the ledgers compress out. Build on what a past session already settled instead of re-deriving it. `/hl:start` recalls recent + on-topic memory right after `next.md` and ahead of the local ledgers, so they corroborate it rather than substitute for it; `/hl:stop` records the session back into it. The memory compounds only if you READ it, not just write it. Where no Dev Memory is configured (e.g. a work machine without autri) this principle is simply inert — the methodology is unchanged.
