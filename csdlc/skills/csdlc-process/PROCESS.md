...
All work items follow a consistent naming scheme that traces back to their epic:

```
{PROJECT}-{EPIC#}-{STORY#}
```

- **Project prefix:** 3-letter code (e.g., `FND` = Foundry, `RTR` = Routr, `QAI` = QuoteAI)
- **Epic:** `E` + number. Epics get a short name: `E9: MCP Agent Surface`
- **Story:** `S` + number within the epic: `FND-E9-S1`
- **Branches:** lowercase with description: `fnd-e9-s1-delete-annotation`
- **Bugs:** `BUG-` prefix scoped to project when relevant: `FND-BUG-7`

This applies to PR titles, branch names, NEXT.md entries, and commit messages. The convention is lightweight — no tracker needed, just consistency.
...
- **Doc-writing agents** — use the [Doc-Writing Agent Template](csdlc-docs/docs/methodology/templates/doc-agent-template.md) for retroactive documentation, doc updates, and code-to-doc audits
...
|------|------------|--------------|
| **Direct sub-agent** | Simple, well-scoped tickets | AI Lead spawns sub-agent with full ticket. Sub-agent implements, verifies, and reports back. |
| **Managed agent** | Complex tickets, parallel batches, audit trail needed | AI Lead spawns a lightweight manager (Sonnet). Manager delegates to Claude Code for implementation, then handles verification, PR creation, and reporting. See the [managed-agents skill](skills/managed-agents/SKILL.md) for the full pattern. |

The managed agent pattern separates orchestration from implementation. The manager (cheap model) handles the paperwork — git, PRs, verification commands. Claude Code (expensive model when needed) handles the actual coding with its purpose-built file exploration and iterative test-fix loops. This also enables **model mixing** — Sonnet managers with Opus coders for complex work, or Sonnet all the way down for well-scoped tickets.

**Hurricane Pattern 🌀:** When executing multiple stories in parallel across repos or worktrees, the AI Lead becomes the "eye of the hurricane" — spawning batches of managed agents, coordinating merges between batches, resolving conflicts, and verifying builds on merged main. Named during the first multi-epic parallel session (March 30, 2026 — 13 stories, 2 repos, ~75 minutes).
