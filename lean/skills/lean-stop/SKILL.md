---
name: lean-stop
description: "End a session cleanly. Triages decisions (active → decisions.md, settled → Foundry, demonstrated → prune), then writes next.md for the next session to pick up cold. Use at the end of every session."
---
...
- **Active** (still being tested, might change) — add to `decisions.md` in the workspace root. Create the file if it doesn't exist.
...
Write `next.md` in the workspace root. The next session starts with zero memory of this conversation — `next.md` is the only bridge.
...
- If there are project-level `decisions.md` updates needed, make them in Phase 1 before writing `next.md`

## Phase 3: Confirm

Tell the human:
- What you wrote in next.md (1-2 sentence summary)
